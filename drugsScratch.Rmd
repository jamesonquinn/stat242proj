...
smallboot = boot(adwithin, getOverlap, R=3)
overlapMeans = matrix(colMeans(smallboot$t),dim(allsets)[1],dim(allsets)[1])

d_inds = seq(dim(genesets)[1]+1,dim(allsets)[1])
g_inds = seq(1,dim(genesets)[1])
rescaler = rbind(cbind(matrix(mean(overlapMeans[g_inds,g_inds]),
                              dim(genesets)[1],
                              dim(genesets)[1]), 
                       matrix(mean(overlapMeans[g_inds,d_inds]),
                              dim(genesets)[1],
                              dim(drugs)[2])),
                 cbind(matrix(mean(overlapMeans[d_inds,g_inds]),
                              dim(drugs)[2],
                              dim(genesets)[1]), 
                       matrix(mean(overlapMeans[d_inds,d_inds]),
                              dim(drugs)[2],
                              dim(drugs)[2])))

rescaled = overlapMeans / rescaler
